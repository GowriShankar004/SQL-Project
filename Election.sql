...
GROUP BY s.state;	
	...
	--Total Seats won by NDA(National Democratic Alliance) Alliance--
SELECT
SUM(CASE
		WHEN party IN (
...
order by Seat_won desc;				
	...
	---Total seats won by INDIA(Indian National Developmental Inclusive Alliance) alliance---
...
SELECT cr.Winning_candidate, pr.Party, cr.Total_Votes, cr.margin, cr.constituency_name, s.state
from constituencywise_results cr INNER JOIN partywise_results pr ON pr.Party_ID= cr.Party_ID
INNER JOIN statewise_results sr ON cr.Parliament_Constituency= sr.Parliament_Constituency
INNER JOIN states s ON sr.State_ID= s.State_ID
WHERE Constituency_Name='EASTDELHI';
...
---Total number of seats won by each party alliance (NDA, INDIA, and OTHER) in each state for the India Elections 2024---
SELECT 
    s.State AS State_Name,
...
WHERE 
    p.party_alliance IN ('NDA', 'INDIA',  'OTHERS')  -- Filter for NDA and INDIA alliances
GROUP BY 
    s.State
ORDER BY 
    s.State;
